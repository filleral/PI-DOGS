...
router.post("/dogs", async (req, res, next) => {
  const {
    ...
    life_span_max,
    image,
    temperament, // arreglo de nombres de temperamentos
  } = req.body;
  try {
    // Crear registro de Dog
    ...
    // Asociar temperamentos
    // if (temperament.length) {
    //   const temperamentPromises = temperament.map(async (tem) => {
    //     try {
    //       // Encontrar o crear registro de Temperament
    //       const [temper, created] = await Temperament.findOrCreate({
    //         where: { name: tem },
    //       });
    //       // Asociar Temperament a Dog
    //       await dogCreated.addTemperament(temper);
    //       console.log(`Temperament ${tem} added to Dog ${dogCreated.id}`);
    //     } catch (err) {
    //       console.log(`Error adding temperament ${tem} to Dog ${dogCreated.id}`);
    //     }
    //   });
